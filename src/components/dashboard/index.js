      ...
      <div className={ this.props.classes.margin }>
        <Grid container spacing={ 24 } >
          ...
          <Grid item xs={ 9 } >
            <Paper className={ this.props.classes.floatingBox }>
              <RouteList
                renderAnnotateButton={ () => this.renderAnnotateButton(firstAnnotationSegment, newAnnotations) } />
            </Paper>
          </Grid>
        </Grid>
        <Grid
  ...
  renderAnnotateButton (segment, count) {
    return (
      <Badge badgeContent={ count } style={{ width: '100%' }} classes={{ badge: this.props.classes.routeListHeaderButtonBubble }}>
        <Button fullWidth variant='outlined' size='large' onClick={ partial(this.goToAnnotation, segment) } className={ this.props.classes.routeListHeaderButton }>
          Begin Annotating
        </Button>
      </Badge>
    )
